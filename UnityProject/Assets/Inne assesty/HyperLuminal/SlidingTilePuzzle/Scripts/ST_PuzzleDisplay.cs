...
public class ST_PuzzleDisplay : MonoBehaviour 
{
    ...
    private GameObject kursorObiekt;
	...
	void Start () 
	{
        ...
        Kursor kursor = FindObjectOfType<Kursor>();
        if (kursor != null)
        {
            kursorObiekt = kursor.gameObject;
	...
	void Update () 
	{
        if (Input.GetKeyDown(KeyCode.P)) // P jak Puzzle
        {
            Complete = true;
        }
        ...
        if (!Cursor.visible)  // jeśli kursor jest niewidoczny
        {
            Cursor.lockState = CursorLockMode.None; // odblokuj kursor
            Cursor.visible = true;                   // ustaw kursor na widoczny
        }
    }
